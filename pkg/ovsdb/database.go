...
func NewDatabase(endpoints []string) (Databaser, error) {
	...
	return &Database{cli: cli,
		uuid:        uuid.NewString(),
		schemas:     make(map[string]string),
		schemaTypes: make(map[string]map[string]map[string]string)}, nil
}

func (con *Database) Lock(ctx context.Context, id string) (bool, error) {
	cnx := context.TODO()
...
func (con *Database) Unlock(ctx context.Context, id string) error {
	cnx := context.TODO()
	session, err := concurrency.NewSession(con.cli, concurrency.WithContext(cnx))
	if err != nil {
		return err
...
func (con *Database) AddSchema(schemaName, schemaFile string) error {
...
func (con *Database) GetData(prefix string, keysOnly bool) (*clientv3.GetResponse, error) {
...
// TODO replace
func (con *Database) GetMarshaled(prefix string, columns []interface{}) (*[]map[string]string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 100*time.Millisecond)
...
func (con *Database) GetSchema(name string) (string, bool) {
	return con.schemas[name], true
}

func (con *Database) GetUUID() string {
	return con.uuid
}

// TODO: LoadServerData is for development purpose only and will be removed in the future
func (con *Database) LoadServerData() error {
	ctx, cancel := context.WithTimeout(context.Background(), 100*time.Millisecond)
	...
	newUUID := func() ovsdbjson.UUID {
		return common.ToUUID(uuid.NewString())
	}

	newSet := func(s interface{}) (*ovsdbjson.OvsSet, error) {
	...
	newMap := func(s interface{}) (*ovsdbjson.OvsMap, error) {
	...
	putNbGlobalOnEtcd := func(uuid string, connections []string, options map[string]string) error {
		...
		putOnEtcd("ovsdb/OVN_Northbound/NB_Global/"+uuid, nbGlobal)
		return nil
	}

	putAclOnEtcd := func(uuid string, action string, direction string, match string, priority int64) error {
		...
		putOnEtcd("ovsdb/OVN_Northbound/NB_Global/"+uuid, acl)
		return nil
	}

	putAddressSetOnEtcd := func(uuid string, addresses []string, externalIds map[string]string, name string) error {
		...
		putOnEtcd("ovsdb/OVN_Northbound/Address_Set/"+uuid, addressSet)
		return nil
	}

	putConnectionOnEtcd := func(uuid string, status map[string]string, target string) error {
		...
		putOnEtcd("ovsdb/OVN_Northbound/Connection/"+uuid, connection)
		return nil
	}

	putForwardingGroupOnEtcd := func(uuid string, childPorts []string, externalIds map[string]string, name string) error {
		...
		putOnEtcd("ovsdb/OVN_Northbound/Forwarding_Group/"+uuid, fowardingGroup)
		return nil
	}

	putGatewayChassisOnEtcd := func(uuid string, chassis_name string, externalIds map[string]string, name string, priority int64) error {
		externalIdsMap, err := newMap(externalIds)
		if err != nil {
			return err
		}
		...
		putOnEtcd("ovsdb/OVN_Northbound/Gateway_Chassis/"+uuid, gatewayChassis)
		return nil
	}

	putloadBalancerOnEtcd := func(uuid string, externalIds map[string]string, name string, options map[string]string, protocol []string, vips map[string]string) error {
		externalIdsMap, err := newMap(externalIds)
		if err != nil {
			return err
		}
		...
		putOnEtcd("ovsdb/OVN_Northbound/Load_Balancer/"+uuid, loadBalancer)
		return nil
	}

	putlogicalRouterOnEtcd := func(uuid string, externalIds map[string]string, name string, options map[string]string, ports []string, staticRoutes []string) error {
		externalIdsMap, err := newMap(externalIds)
		if err != nil {
			return err
		}
		...
		putOnEtcd("ovsdb/OVN_Northbound/Logical_Router/"+uuid, logicalRouter)
		return nil
	}

	putlogicalSwitchOnEtcd := func(uuid string, externalIds map[string]string, name string, ports []string) error {
		externalIdsMap, err := newMap(externalIds)
		if err != nil {
			return err
		}
		...
		putOnEtcd("ovsdb/OVN_Northbound/Logical_Switch/"+uuid, logicalSwitch)
	...
	err := putNbGlobalOnEtcd("a5088a51-7756-4dd4-909c-b7c59c9fcce7", []string{"413afe3e-79ff-4583-88a6-f02b70b8e927"},
		map[string]string{
	...
	err = putAclOnEtcd("aa2bab19-9b31-4d01-b1ad-f5e49dd269f8", "allow-related", "to-lport", "ip4.src==10.244.0.2", 1001)
	if err != nil {
		return err
	}

	err = putAclOnEtcd("3ed181f9-7c68-47ee-bcdc-6cf393a02772", "allow-related", "to-lport", "ip4.src==10.244.1.2", 1001)
	if err != nil {
		return err
	}

	err = putAclOnEtcd("7071b927-cc6d-4145-8849-395e6226fdac", "allow-related", "to-lport", "ip4.src==10.244.1.2", 1001)
	if err != nil {
		return err
	}
	...
	err = putAddressSetOnEtcd("532757d0-bc2e-41b9-bafe-2542f995b011", []string{"10.244.0.5"}, map[string]string{"name": "local-path-storage_v4"}, "a10956707444534956691")
	if err != nil {
		return err
	}

	err = putAddressSetOnEtcd("8e33c234-2da4-4e5f-858f-4bcd5bc3c68b", []string{}, map[string]string{"name": "local-path-storage_v4"}, "a5154718082306775057")
	if err != nil {
		return err
	}

	err = putAddressSetOnEtcd("3581fd85-1428-45a8-9702-edec71dda0a1", []string{"10.244.0.3", "10.244.0.4"}, map[string]string{"name": "kube-system_v4"}, "a6937002112706621489")
	if err != nil {
		return err
	}

	err = putAddressSetOnEtcd("99ad8ae1-bc86-4662-bca4-a88fd675ee3d", []string{}, map[string]string{"name": "ovn-kubernetes_v4"}, "a5675285926127865604")
	if err != nil {
		return err
	}

	err = putAddressSetOnEtcd("fde500ad-eff5-47a3-be0b-02e7c23a1357", []string{}, map[string]string{"name": "kube-public_v4"}, "a18363165982804349389")
	if err != nil {
		return err
	}

	err = putAddressSetOnEtcd("0af13342-2ea7-486d-825a-b57bd70a8cbc", []string{}, map[string]string{"name": "kube-node-lease_v4"}, "a16235039932615691331")
	if err != nil {
		return err
	}
	...
	err = putConnectionOnEtcd("413afe3e-79ff-4583-88a6-f02b70b8e927", map[string]string{"bound_port": "6641", "n_connections": "3", "sec_since_connect": "0", "sec_since_disconnect": "0"}, "ptcp:6641:172.18.0.4")
	if err != nil {
		return err
	}
	...
	err = putForwardingGroupOnEtcd("6be9235a-b3b6-41d7-a5aa-356b5b3c96cc", []string{"25f2e69e-4bac-4529-9082-9f94da060cf1",
	...
	err = putForwardingGroupOnEtcd("ee4d82d2-3a7d-4737-be8d-656374f5d56c", []string{"b4298483-cf17-46d4-9da1-034eab065ff1",
	...
	err = putGatewayChassisOnEtcd("99c45e0b-3688-4992-900c-7d5a25930ba3", "1bd76edb-8626-4ecd-8185-788bd2121bda",
	...
	err = putloadBalancerOnEtcd("32cc16f0-cda7-4c63-87d7-d30349ce32d7",
	...
	err = putlogicalRouterOnEtcd("22c3143c-15c8-4018-91b6-8fe4b0ffab80",
	...
	err = putlogicalRouterOnEtcd("70f7ff95-16c1-4832-9073-5c5ba807d205",
	...
	err = putlogicalSwitchOnEtcd("4cccc9ad-5ba6-42f9-a749-2d5f3c54ace4", map[string]string{}, "ext_ovn-control-plane",
		[]string{"006d780f-7169-44f6-8eb2-9df757feef61", "a8fe81ba-4746-4d6f-aebe-d53cfd4ae46f"})
	if err != nil {
		return err
	}

	err = putlogicalSwitchOnEtcd("1822aa4e-9d58-4261-a22b-ecb6f864a3bc", map[string]string{}, "ext_ovn-worker",
...
func (con *DatabaseMock) LoadServerData() error {
	return con.Error
}
